...
pub(crate) trait IRFft {
    ...
    fn recombine(x: &mut [Complex32]) {
        ...
        //
        // Elimination method can be used to solve for x0.re and x0.im:
        // packed.re = x0.re + x0.im
        // packed.im = x0.re - x0.im
        //
        // Solving for x0.re:
        // packed.re + packed.im = (x0.re + x0.im) + (x0.re - x0.im) = 2 * x0.re
        // Therefore: x0.re = (packed.re + packed.im) / 2
        //
        // Solving for x0.im:
        // packed.re - packed.im = (x0.re + x0.im) - (x0.re - x0.im) = 2 * x0.im
        // Therefore: x0.im = (packed.re - packed.im) / 2
        let x0 = x[0];
        ...
        for k in 1..u {
            ...
            // sum.re = (x_k.re + x_nk.re) / 2
            // diff.im = (x_k.im - x_nk.im) / 2
            ...
            // sum.im = (A * twiddle_re + B * twiddle_im) / (twiddle_re^2 + twiddle_im^2)
            // diff.re = (A * twiddle_im - B * twiddle_re) / (twiddle_re^2 + twiddle_im^2)
            // Where A = (x_k.re - x_nk.re) / 2, B = (x_k.im + x_nk.im) / 2
