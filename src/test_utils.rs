...
pub mod approx {
    ...
    pub fn complex_eq(a: Complex32, b: Complex32, tolerance: f32) -> bool {
        ...
        let abs_a = (a.re * a.re + a.im * a.im).sqrt();
        let abs_b = (b.re * b.re + b.im * b.im).sqrt();
        let max_abs = abs_a.max(abs_b);

        let rel_diff_re = if max_abs > 1.0 {
            diff_re / max_abs
        } else {
            diff_re
        };
        let rel_diff_im = if max_abs > 1.0 {
            diff_im / max_abs
        } else {
            diff_im
        };

        rel_diff_re < tolerance && rel_diff_im < tolerance
    }
